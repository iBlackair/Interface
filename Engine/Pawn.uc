...
var transient bool		bIsScenePawn; //신에디터에서 사용된 폰인지
...
struct native NAttackActionParam
{
	...
	var int		JewelItemClassID;  // JEWEL 추가 - by y2jinc (2013. 9. 4)
...
// WeaponType 별로 Animation이 따로 존재하는 구조이다.
// WeaponType 을 추가할경우
// 1. PawnAnimData.h에 있는 WEAPONTYPE_NUM 숫자를 맞춰줘야한다.
// 2. PawnSheathingMgr.uc에 있는 WEAPONTYPE_CNT의 숫자를 맞춰줘야한다.
// 3. 무기타입을 추가할 경우 L2GameData.cpp에 있는 Charagrp.txt에서 무기별 sound 읽어오는 부분을 추가해주어야한다
// 4. 인챈트 이펙트 붙이는 게 무기별로 다르기때문에 AbnormalProcess.cpp에서 추가해주어야한다
// 5. 활의 경우 BOW와 CROSSBOW 예외처리된 곳을 추가로 다 해주어야한다(화살의 장전 등)
// 6. 무기타입을 추가할 경우 L2GameData.cpp에서 정령탄이펙트 붙일 곳을을 soulshot.int에서 읽어오는 부분이 있다(WeaponString[]. 그게 무기타입별 문자열로 되어있는데 그걸 추가해줘야한다.
enum WeaponType
{
	WT_HAND,
	WT_1HS,
...
var bool				bDemageAct;
//#ifdef GSTAR2010 피격자 애니메이션 시스템이 들어가면 없애야함
...
//solasys-보물상자 애들이 물속에서 run 애니메이션을 하기 때문에 수정해볼라구
...
var int CHState; //의문의저택상태

// ProjectedTargetEffect 관련
...
var bool bRelativeSoundRadius;	// true일 경우, Drawscale에 따라 Sound의 Radius를 조절한다.
...
//2009/10/13  by elsacred // Pawn의상태가 바뀌면 호출..  
var int PawnState;

// #ifdef __L2	lancelot 2009. 11. 20.  actor에 있던걸 옮겨옴
...
//unr에 폰을 배치한후 맵과 함께 저장하고 싶다면 true로 하자.
var() bool IsClientSidePawn;

// jylee, 구시대의 유물. 아래 ClientSidePawnAnimList 로 인해 복수사용이 가능하므로 언젠가는 삭제해야함
// 근데 기존에 쓰고 있던 폰들이 있어서 갑자기 빼면 기록이 사라짐
// APawn.Serialize() 에다가 이걸 사용하고 있으면 List로 옮기고 이건 None으로 바꾸도록 해놨으므로
// 시간이 지난 후 삭제 요망.
var name ClientSidePawnAnim; 

//unr에 배치된 폰의 경우 로드된 메쉬의 이름을 unr을 저장할때 pawn에 저장해 놓는다. 나중에
//pawn이 serialize될때 메쉬가 같이 로드된다
var name SavedMeshName;

// 듀얼캐스팅에 사용되는 팔의 블랜딩을 위해서 bone index를 저장 - lancelot 2011. 1. 31.
...
//jylee, hairshop test
var	transient color		DynamicHairColor; // hairex 사용 시의 color vector
var	transient bool		bUseDynamicHairColor; // 이 머리가 색변경 가능한지와는 별개로, 기본 색을 쓸건지 아닌지 여부
...
var int	CurAtkAnimIndex;		// 평타 애니메이션 순차적재생시 현재 애니메이션 인덱스
...
native function name GetWalkAnimName();				//걷기
native function name GetRunAnimName();						//뛰기
...
native function name GetWaitAnimName();					//일반대기
...
native function name GetAtkWaitAnimName();				//공격대기
//function name GetSitAnimName() {return SitAnimName[CurWeaponType];}						//서있다가 앉는모션
native function name GetSitWaitAnimName();				//바닥에 앉아서 대기중
...
//}					
native function name GetDeathWaitAnimName();		//누워있는 모션
//{ 
...
// 마법캐스팅 A조합부터 L까지 랜덤 발동을 위해 추가 애니메이션 7개 lancelot 2009. 10. 19.
