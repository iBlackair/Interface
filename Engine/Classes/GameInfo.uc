...
function ChangeMap(int ContextID) // sjs
{
    ...
    MyList = spawn(ML);
    MapString = MyList.Maps[ContextID];
    MyList.Destroy();
...
event InitGame( string Options, out string Error )
{
    ...
    if (InOpt != "" && CurrentGameProfile == none)
    {
		...
		{
			CurrentGameProfile.bInLadderGame = false;
		}
    }
...
{   
    ...
    */
    ...
    // jij --- insert tabs for proper parsing
    return
        Level.ComputerName
    $   " "
    $   Left(Level.Title,24) 
...
/*
	if ( GameStats != None )
	{
        Error = GameMessageClass.Default.MustHaveStats;
        return;
    }
*/		
...
event PostLogin( PlayerController NewPlayer )
{
    ...
    if( SongName != "" && SongName != "None" )
		...
		log( "Map"@Level@"does not have an associated MUSIC track",'Error');
...
static function FillPlayInfo(PlayInfo PlayInfo)
{
	...
	i=0;
	PlayInfo.AddSetting("Bots",   "GameDifficulty",     default.GIPropsDisplayText[i++], 0, 50, "Select", default.GIPropsExtras[0], "Xb");
	...
	PlayInfo.AddSetting("Game",   "GoreLevel",          default.GIPropsDisplayText[i++], 0, 120, "Select", default.GIPropsExtras[1]);
	...
	if (default.MutatorClass != "")
	{
		...
		if (mutClass != None)
		{
			mutClass.static.FillPlayInfo(PlayInfo);
			PlayInfo.PopClass();
		}
	}
}
...
{
	return BaseMutator.RecommendCombo(ComboName);
}
...
defaultproperties
{
     ...
     GIPropsExtras(0)="0;Novice;1;Average;2;Experienced;3;Skilled;4;Adept;5;Masterful;6;Inhuman;7;Godlike"
     GIPropsExtras(1)="0;Full Gore;1;Reduced Gore"
}
