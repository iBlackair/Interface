...
replication
{
    ...
    unreliable if( Role==ROLE_Authority )
        SetFOVAngle, ClientDamageShake, ClientFlash,  
    ...
    unreliable if( Role<ROLE_Authority )
        ShortServerMove, ServerMove, RocketServerMove, Say, TeamSay, ServerSetHandedness, ServerSetAutotaunt, ServerViewNextPlayer, ServerViewSelf,ServerUse, ServerDrive;
    reliable if( Role<ROLE_Authority )
...
exec function LogPhysics()
{
	local Actor A;
	
	ForEach DynamicActors(class'Actor',A)
		if ( (A.Physics != PHYS_None) && (A.Physics != PHYS_Rotating) )
			log(A$" physics "$A.physics);
}
...
event PostBeginPlay()
{
    ...
    if ( Level.NetMode != NM_DedicatedServer )
		UpdateForceFeedbackProperties( ForceFeedbackSupported(), class'PlayerController'.default.bEnableWeaponForceFeedback, class'PlayerController'.default.bEnablePickupForceFeedback, class'PlayerController'.default.bEnableDamageForceFeedback, class'PlayerController'.default.bEnableGUIForceFeedback );
}
...
function ClientSetHUD(class<HUD> newHUDClass, class<Scoreboard> newScoringClass )
{
    if ( myHUD != None )
        myHUD.Destroy();
...
final function SavedMove GetFreeMove()
{
    ...
    if ( FreeMoves == None )
    {
        // don't allow more than 60 saved moves
...
)
{
    ...
    NetMoveDelta = FMax(80.0/Player.CurrentNetSpeed, 0.025);
...
exec function JoeMenuTest(string NewMenu)
{

	log("#### Player.GUIController:"@Player@Player.GUIController);

	if (Player.GUIController!=None)
	{
		log("#### Sending OpenMenu call");
		Player.GUIController.OpenMenu(NewMenu);
	}
	else
		log("Player does not have a GUIController object");
}
